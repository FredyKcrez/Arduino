...
int myArreglo[200]; /* Vector que contendra los puntos graficados según coordenadas, el valor 81 sale de restaler 4 pixeles a la dimensión
...
int j, i=0; //otro contador para recolectar y recorrer el arreglo, pero utilizado cuando la pantalla a un esta llena horizontalmente.
int escala = 47.0/1023;
...
void loop() {
  for(yCtrl=0; yCtrl<=80; yCtrl++) { /* for para recorrer, llenar la matriz, y mostrar el(los) pixel(es) en la pantalla, recorre
    ...
    myArreglo[i] = posY*escala;// mapearlo en escala vertical de la pantalla 0-47 (dimensión vertical de la pantalla)
    if(i>=200) {
      endSketch();
    }
    display.drawPixel(yCtrl+4, myArreglo[i], BLACK); /* dibujado de pixeles, a yCtrl se le aumenta en uno para que comience a dibujar
...
void movPantalla() {
  ...
  for(j=z; j<=z+79; j++) { //recorremos el arreglo, consultando los valores que contiene.
    ...
    display.drawPixel(j-z+4, myArreglo[j], BLACK); //re-dibujado de pixeles
  }
}

void endSketch() {
  display.println("Fin del\nskecth");
  display.display();
  delay(100000);
}
